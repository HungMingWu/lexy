...
For that, we need to use a {{% docref choice %}} between the previous `dsl::times` rule and {{% docref "dsl::lit" %}}`("unreleased")`.
...
Luckily, `dsl::lit` is already a branch rule, so we can turn the number part into one by using {{% docref "lexy::dsl::else_" %}}:
...
struct config
{
    static constexpr auto rule = []{
        ...
        auto name_field    = make_field(dsl::lit<"name">, dsl::p<name>); <3>
        auto version_field = make_field(dsl::lit<"version">, dsl::p<version>);
        auto authors_field
            = make_field(dsl::lit<"authors">, dsl::p<author_list>);
...
The solution is to use the {{% docref "lexy::as_aggregate" %}} callback together with {{% docref "lexy::mem" %}}.
Using `lexy::mem<&struct::field> = rule` in the DSL assigns the value of `rule` to the member `field` in `struct`,
...
struct config
{
    ...
    static constexpr auto rule = [] {
        ...
        auto name_field    = make_field(dsl::lit<"name">, lexy::mem<&PackageConfig::name> = dsl::p<name>); <2>
        auto version_field
...
<2> Each rule now contains the assignment to the appropriate member using {{% docref "lexy::mem" %}}.
