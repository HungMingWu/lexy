...
Writing the same rule in lexy using the {{% docref "dsl::lit" %}} primitive rule and the {{% docref choice %}} combinator, results in this:
...
The correct way to write it would thus be `dsl::lit<"ab"> | dsl::lit<"a">`.
...
So the choice above `dsl::lit<"ab"> | dsl::lit<"a">` compiled because both alternatives are literal rules, which can be efficiently checked.
...
struct color
{
    static constexpr auto rule = []{
        ...
        auto fnc_color = dsl::lit<"rgb"> + dsl::parenthesized(dec_channels); <5>
...
For example, consider a call to `dsl::while_(dsl::lit<"a"> >> dsl::lit<"b"> + dsl::lit<"c">)`.
