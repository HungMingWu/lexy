...
{{% playground-example "expected_literal" "`dsl::lit()` raises an expected literal error" %}}
...
`production()`::
  ...
  This is the first non-{{% docref "lexy::transparent_production" %}} parent of the current production.
`position()`::
