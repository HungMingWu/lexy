...
TEST_CASE("_detail::type_name")
{
    SUBCASE("function")
    {
        ...
        CHECK(lexy::_detail::type_name<type>() == std::string_view("some type"));
    }
    SUBCASE("variable")
    {
        CHECK(lexy::_detail::type_name<test_type>() == std::string_view("some type"));
    }
#if LEXY_HAS_CONSTEXPR_AUTOMATIC_TYPE_NAME
    SUBCASE("automatic")
    {
        CHECK(lexy::_detail::type_name<int, 0>() == std::string_view("int"));

        CHECK(lexy::_detail::type_name<ns::test_type, 0>()
              == std::string_view("ns::test_type"));
        CHECK(lexy::_detail::type_name<ns::test_class, 0>()
              == std::string_view("ns::test_class"));
        CHECK(lexy::_detail::type_name<ns::inner::test_type, 0>()
              == std::string_view("ns::inner::test_type"));

        CHECK(lexy::_detail::type_name<ns::test_type>() == std::string_view("test_type"));
        CHECK(lexy::_detail::type_name<ns::test_class>()
              == std::string_view("test_class"));
        CHECK(lexy::_detail::type_name<ns::inner::test_type>()
              == std::string_view("inner::test_type"));
