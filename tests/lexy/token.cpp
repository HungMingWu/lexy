...
TEST_CASE("token_kind")
{
    SUBCASE("void")
    {
        ...
        CHECK(value.get() == 0);
        CHECK(value.name() == std::string_view("token"));
        CHECK(value == 0);

        lexy::token_kind period(lexy::dsl::period);
        CHECK(!period);
        CHECK(period.is_predefined());
        CHECK(period.name() == std::string_view("token"));
        ...
        CHECK(manual.get() == 42);
        CHECK(manual.name() == std::string_view("token"));
        CHECK(manual == 42);
    ...
    SUBCASE("enum")
    {
        ...
        CHECK(value.name() == std::string_view("a"));
        ...
        CHECK(period.name() == std::string_view("c"));
        CHECK(period == token_kind::c);
        CHECK(period == lexy::dsl::period);

        lexy::token_kind<token_kind> manual(lexy::dsl::period.kind<token_kind::b>);
        INFO(int(lexy::token_kind_of<std::decay_t<decltype(lexy::dsl::period.kind<token_kind::b>)>>));
        CHECK(manual);
        CHECK(!manual.is_predefined());
        CHECK(manual.get() == token_kind::b);
        CHECK(manual.name() == std::string_view("b"));
...
TEST_CASE("token")
{
    ...
    SUBCASE("void")
    {
        ...
        CHECK(zero.kind() == 0);
        CHECK(zero.name() == std::string_view("token"));
    ...
    SUBCASE("enum")
    {
        ...
        CHECK(b.name() == std::string_view("b"));
