...
namespace ast
{
    struct address
    {
        ...
        {
            if (display_name)
                std::printf("%s <%s@%s> ", display_name->c_str(), local_part.c_str(),
                            domain.c_str());
            else
...
namespace grammar
{
    ...
    struct dot_atom
    {
        // A list of atom separated by periods which are part of the content and thus captured
        // (unlike the whitespace).
