...
namespace grammar
{
    ...
    struct execute_body
    {
        ...
        static constexpr auto rule
            = dsl::curly_bracketed.opt_list(dsl::recurse<statement<Vars...>>);
        static constexpr auto value = lexy::forward<void>;
    };
    ...
    struct if_stmt
    {
        ...
        struct impl
        {
            ...
            static constexpr auto rule = [] {
                auto non_zero
                    = dsl::p<execute_body<Vars...>> + dsl::if_(kw_else >> dsl::p<skip_body>);
    ...
    struct while_stmt
    {
        ...
        static constexpr auto rule
            = kw_while >> (dsl::p<impl<Vars>> | ... | unknown_variable_error);
        static constexpr auto value = lexy::forward<void>;
    };
    ...
    struct program
    {
        ...
        static constexpr auto rule = [] {
            ...
            auto run
                = dsl::loop(dsl::eof >> dsl::break_ | dsl::else_ >> dsl::p<statement<Vars...>>);
...
int main(int argc, char** argv)
{
    ...
    auto result = lexy::parse<program>(file.buffer(), lexy_ext::report_error.path(argv[1]));
    if (!result)
        return 2;
