...
namespace grammar
{
    ...
    struct varint
    {
        ...
        static constexpr auto value = [] {
            ...
            // As sink, we create a builder and call `add_prefix_byte()` for every byte that is part
            // of the prefix list.
    ...
    struct field : lexy::scan_production<ast::field>
    {
        ...
        static scan_result scan(lexy::rule_scanner<Context, Reader>& scanner)
        {
            ...
            default:
                ...
                // Calling `fatal_error()` will put the scanner in a failed state, caught by the
                // check below.
